...
root's replies with it. Administrators review reports in the studio's **Site administration** section,
which grants no access to private Git content. Reports contain at most 1,000 code points.
...
The management page lists the signed-in account's spaces. Select a space before editing; its `workspace` URL parameter keeps separate tabs independent. The studio's **Account** section also supports connecting an existing private GitHub/CNB repository, querying or retrying an interrupted creation, and accepting a workspace invitation. Configure `POKETTO_REPOSITORY_CREDENTIAL_KEY` as a Base64-encoded 32-byte secret before enabling repository connection. New spaces start with public delivery disabled. Creation commits the content template into an empty repository as its first commit; if that write does not complete, the space's **Storage** section offers it. A repository that already holds content is left unchanged, and the same section lists the template's absent guide and policy files with an action that adds only those. Provider tokens require metadata read and Git write access and are never retained in browser drafts.
...
Markdown metadata is optional, and unchanged source bytes are retained. Default routes omit `public/` and `.md`; an explicit article route is preserved but cannot grant publication rights. Folder landings always derive their route from their directory and ignore a frontmatter route override, including after moves; authored source remains intact. The public detail endpoint is `GET /api/public/document?route=...`; list/search and tags include snapshot metadata. `index.md`, or `README.md` when no eligible valid index exists, owns its folder route (`public/index.md` owns `/`) and supplies a non-recursive sibling-image gallery without repeating body images. When both valid landings exist, structured reading uses the index and retains the README source. Gallery images open in a dialog with previous/next buttons, arrow-key navigation and Escape return to the opener. Public gallery tiles load thumbnails up to 640 pixels on the longest side; opening the lightbox loads the unchanged original. Transparent thumbnails use PNG and opaque thumbnails use JPEG. EXIF orientation is applied to JPEG, PNG and WebP sources, and animated previews use the first frame. WebP thumbnails accept up to four million source pixels; an invalid or repeated EXIF payload leaves the image without a thumbnail. An unavailable preview still opens its authorized original. Authenticated editor previews retain original image delivery.

To schedule a publishable article, add `publish_at` to its frontmatter, either as `YYYY-MM-DD` (00:00 UTC) or as an offset timestamp such as `2026-10-01T09:00:00+08:00`. Until that instant the article is absent from every anonymous surface: pages, listings, tags, archives, discovery, search, sitemaps, feeds, covers, image grants and community. It appears at the instant without a new commit. Without `created_at` or `date`, its date becomes the `publish_at` value, so it lists as new. An unparseable value reports `INVALID_MARKDOWN` and keeps the article unpublished. The key never publishes anything outside `public/`. Space members who can read the public scope see the file in the studio before it is due, and the editor shows **定时发布** with the time. Full-read execution copies include it; public-only execution copies omit it until it is due. See [scheduled publishing](../notes/implemented/2026-09-24-scheduled-publishing.md).

Space owners can turn on **公开修订历史** in the website settings; it is off by default. While it is on, every article footer links **修订历史**, and `/s/{slug}/history/{route}` lists the article's public bodies oldest first and compares any two line by line. Walking first parents back from the served commit, history ends at the first commit where the article was not publishable at the same path and route: publication disabled or the path excluded, the file absent, over 1 MiB or unparseable, another route, or a `publish_at` still in the future. Consecutive commits with the same body form one version dated by the earliest. Commit ids, messages, identities and frontmatter are never shown. A read scans at most 256 commits, 50 versions and 2 MiB of UTF-8 bodies within two seconds, with two reads at a time. A bound that stops it, or an earlier version over 1 MiB or unparseable, marks the history incomplete, and the page says earlier versions are not listed. History pages are `noindex`. `GET /api/public/spaces/{slug}/history?route=…` answers 404 while the setting is off. See [public revision history](../notes/implemented/2026-09-24-public-revision-history.md).
...
Homepage cards distinguish articles, directories, albums and collections. An article card shows its first public inline image as a cover when one is readable, and so does a directory whose folder holds no further images. Each public article also has a stable cover address, `/s/{slug}/cover/{route}`, which link previews and structured data use: it serves the same cover thumbnail, prepared on each request, redirects to the site's `/share.png` when the article has no cover, and answers 404 once the article is no longer public. Article pages describe themselves to search engines as `BlogPosting` structured data, and the homepage as a `WebSite` with its search. A folder with both sibling images and an authored reading sequence shows both album and collection labels. Visible album cards load one thumbnail linking to the named folder; an unavailable cover keeps that entrance without downloading an original. Revisiting a current batch page refreshes its short-lived cover URLs while retaining the card order.
...
without advancing file baselines; run `poketto sync` before saving affected
paths.
...
in the capturing lease. `get_artifact` serves it to any MCP session of the same
account, workspace and reading scope while that lease stays open; a request under
another credential of the account moves the copy to a new lease, which closes the
old lease and its artifacts. It renders validated raster images
or returns text/binary pages; long command output also supplies artifact handles.
Handles expire after five minutes or when the lease closes, and do not upload, save or
publish files. Resource limits and cancellation close the lease, so
...
Every verified `main` commit publishes separate Spring and frontend images from the same source commit. Copy the files under `deploy/` and a filled-in `.env.example` as `.env` into the host's deployment root. Supply the private domain/DNS configuration, repository/database credentials, separate data directories and four image pins. Run `deploy.sh --app-image <application-image> --app-revision <commit> --frontend-image <frontend-image>`; later runs without options redeploy the recorded pins. No initialization token is configured: once the application is running, create the first administrator with `./deploy.sh --initialize-admin` in an interactive terminal. Both application revision labels must match, and PostgreSQL/Caddy references must carry registry digests.
