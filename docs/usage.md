...
Registration invitations are separate from workspace invitations. `POKETTO_REGISTRATION_USER_INVITATIONS_ENABLED` defaults to `false`, allowing only site administrators to issue registration invitations; `true` also permits ordinary accounts. No fixed per-user quota is imposed. Issuers can list and revoke their own codes even after issuance is disabled. The account and registration HTTP endpoints are documented in [Registration invitations](../notes/implemented/2026-09-11-registration-invitations.md#http-contract); their browser workflow is part of the [multi-user delivery proposal](../notes/proposed/2026-09-11-multiuser-workspaces-and-discovery.md).
