...
Homepage cards distinguish articles, directories, albums and collections. An article card shows its first public inline image as a cover when one is readable. A folder with both sibling images and an authored reading sequence shows both album and collection labels. Visible album cards load one thumbnail linking to the named folder; an unavailable cover keeps that entrance without downloading an original. Revisiting a current batch page refreshes its short-lived cover URLs while retaining the card order.
