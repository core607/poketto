...
已登录的社区成员可以在公开文章下方点「建议修改」：直接修改页面上的正文，可附上最多 500 字的说明，并选择采纳后是否署名：在文章底部致谢昵称，并在仓库提交里记录账号编号。
以下情况会拒绝提交：页面载入后文章已经改过、建议没有任何改动、提议人与空间所有者之间有屏蔽，或提议人对这篇文章已有一条待处理的建议；频率上限为每分钟 5 条、每天 30 条。
空间所有者会收到通知。持有 `PUBLISH` 权限的成员在工作台的「读者勘误」里以逐行对比审阅待处理的建议。
采纳时会在远端最新提交上重新读取文件，只有正文仍与建议的基础一致才替换正文，frontmatter 原样保留；提议人同意署名时，提交带 `Poketto-Suggested-By` 标注；否则建议标记为过期。采纳进行中时，不采纳或撤回会返回冲突；十分钟内没有完成的采纳重新算作待处理。
不采纳不会改动任何内容。无论结果如何提议人都会收到通知，待处理的建议也可以由提议人撤回。
同意署名的提议人被采纳后，只要这个地址上仍是同一篇文章，就会在文章底部获得致谢。已处理建议的正文和说明在 90 天后清除。
