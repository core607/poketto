...
文章和已授权预览中的相对 Markdown 链接，例如指向 `recording.mp3` 的链接，可以为索引中的 MP3、WAV、MP4 和 WebM 原件显示原生播放控件。导入时使用 `audio/mpeg`、`audio/wav`（也接受 `audio/wave` 和 `audio/x-wav`）、`audio/mp4`、`video/mp4`、`audio/webm` 或 `video/webm`。播放会校验原件完整性和有界的容器签名，具体编码仍取决于浏览器支持；不自动播放或预加载，失败时可使用旁边的下载链接。外链、原始 HTML 和不支持的原件不会成为播放器。
