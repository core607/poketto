...
已登录的社区成员可以在公开文章下方点「建议修改」：直接修改页面上的正文，并可附上最多 500 字的说明。
...
采纳时会在远端最新提交上重新读取文件，只有正文仍与建议的基础一致才替换正文，frontmatter 原样保留，提交带 `Poketto-Suggested-By` 标注；否则建议标记为过期。
不采纳不会改动任何内容。无论结果如何提议人都会收到通知，待处理的建议也可以由提议人撤回。
被采纳的提议人会在文章底部获得致谢。已处理建议的正文和说明在 90 天后清除。
