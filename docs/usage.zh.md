...
已登录的社区成员可以在公开文章下方点「建议修改」：直接修改页面上的正文，可附上最多 500 字的说明，并选择采纳后是否公开署名致谢。
...
采纳时会在远端最新提交上重新读取文件，只有正文仍与建议的基础一致才替换正文，frontmatter 原样保留，提交带 `Poketto-Suggested-By` 标注；否则建议标记为过期。采纳进行中时，不采纳或撤回会返回冲突。
