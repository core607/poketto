...
首页卡片区分文章、目录、相册和合集。文章卡片会把正文里第一张可以公开读取的图片作为封面。文件夹同时包含同目录图片和编排好的阅读顺序时，会同时显示相册、合集标签。当前页的相册卡片加载一张缩略图，点击进入对应文件夹；封面不可用时保留入口，不自动下载原图。重新打开仍有效的批次页面会刷新短期封面地址，卡片顺序保持不变。
