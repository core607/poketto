...
`/sitemap.xml` 为站点首页和所有已开启的空间网站提供索引。子站点地图从各空间当前获准发布的快照列出规范的 `/s/{slug}` 和 `/s/{slug}/read/...` 地址，不抽样使用发现批次。快照不可用或枚举超限时返回 503，不交付不完整列表；未知或关闭的空间返回 404。`/robots.txt` 使用 `POKETTO_PUBLIC_URL` 声明该索引，并建议爬虫避开 `/admin`、`/api/` 和首页批次地址（`/?batch=`、`/?afterBatch=`）。这些抓取指令不授予或撤销内容访问权限。
