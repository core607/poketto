...
test("agent reserves final submission and returns tool errors for calls beyond the search budget", async () => {
  const f = fixture();
  let closes = 0;
  let executions = 0;
  try {
    f.engine.models.chat = async (
      ...
      if (requiredTool) {
        ...
        assert.equal(messages.filter((m) => m.role === "tool").length, 15);
        return {
          tool_calls: [
            {
              id: "done",
