...
Run this full workflow only when the user invokes `translate-docs` by name. Routine edits to the public README, requirements or usage-reference pair follow one rule without loading the extended workflow: **when one side changes, make the smallest corresponding change to the other side in the same change**.
