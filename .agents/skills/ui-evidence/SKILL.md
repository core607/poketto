...
1. Record the exact state being demonstrated: a clean worktree at a commit SHA, or the base SHA plus staged and unstaged diffs and relevant untracked files before commit. Keep that state unchanged during the run; isolate unrelated changes without discarding user work.
