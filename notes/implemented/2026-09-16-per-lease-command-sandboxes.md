...
One systemd command unit per lease. The lease already is the session boundary: the application keeps one lease per account, workspace and reading scope, renews it while the worker is reachable, reuses it across commands and transports, and closes it only on capacity pressure, another client fencing the account, cancellation, revocation, worker restart or shutdown ([IsolatedRepositoryExecutor](../../src/main/java/io/github/core607/poketto/executor/internal/IsolatedRepositoryExecutor.java)).
...
- `WorkerSocketTests` verifies freshness propagation without changing copy identity. `WorkerAnswerTests` rejects missing or non-boolean freshness and missing protocol markers. Worker tests validate `idleUnitSeconds`, idle lease renewal, output bounds and command-epoch fencing.
- The native worker fixture measured 40,878,080 bytes of idle cgroup memory and an 11.41 ms mean across twenty reused `true` commands. Twenty mixed Git/search/Python commands averaged 49.89 ms in a separate fixture. These local worker measurements exclude HTTP and model/client latency; they are not an end-to-end service benchmark.
- The [native worker probe](../../executor-service/native_probe.py) provides `--lease-sandbox-only` for state continuity, idle captures and resets, and `--baseline-only` for baseline installation and helper containment. The [Java adapter probe](../../executor-native/README.md) exercises the protocol, authorized CLI bridge and account-copy lifecycle. These fixtures use synthetic identity and Git authority; they do not replace deployed HTTPS/MCP acceptance.

## Same-topic audit

[Account working copies](../implemented/2026-09-14-account-working-copies.md) is retained; this note is the measurement it deferred. [Command timeout and explicit copy disposal](../implemented/2026-09-14-ephemeral-copy-lifecycle.md) is retained: a timeout still keeps the copy, and the reset is now reported explicitly while successful commands retain `/tmp`. The [local execution supervisor](../implemented/2026-09-05-local-execution-supervisor.md) keeps the privileged boundary this note builds on; its per-command process tree is what this note replaces.
