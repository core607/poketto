...
- `./gradlew test checkstyleMain checkstyleTest checkstyleIntegrationTest spotlessCheck` passes: 473 unit tests including the module-boundary suite, and Checkstyle on all three source sets with four fewer exemptions. One test is new; none was modified.
- The two relative-link algorithms were compared over 48,205 generated path pairs of the shapes both callers produce. They agree on every pair except where the target is the document's own directory, which cannot occur because a target is always a file. A document at the repository root is the one input the replaced algorithm could not answer at all, and `RepositoryPatchServiceTests.aMoveRepairsALinkWrittenByAMarkdownFileAtTheRepositoryRoot` pins that a move repairs a link written by a root file.
