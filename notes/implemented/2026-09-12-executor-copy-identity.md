...
A response-only `fresh` flag arrives after a potentially harmful command. Comparing only commits misses fresh clones at the same revision. Automatically selecting another copy by identity and workspace merges independent chats. Those alternatives do not provide the execution precondition. The required request field intentionally changes the development API; callers must retain the returned ID. [Account working copies](2026-09-14-account-working-copies.md) implements durable work directories, shared ownership and pending-write retention while preserving explicit copy preconditions.
