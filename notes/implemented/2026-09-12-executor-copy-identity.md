...
[Combined native evidence](../../executor-native/evidence/2026-09-12-copy-identity.json) records 34 passing Java/SRT scenarios, process-loss lease expiry and cleanup, with verified runtime and worker hashes. Focused executor/MCP tests, the native Linux storage replay and worker/CLI tests also pass on that source. The receipt states the synthetic-authentication and injected-idle boundaries.
