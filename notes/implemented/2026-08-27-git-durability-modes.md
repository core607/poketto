...
- Per-workspace operational state exposes whether a remote is configured, `local_head`, `last_mirrored_commit`, mirror lag in commits and duration, the last attempt time, and a sanitized failure category. A workspace whose repository names no remote reports replication as off rather than failing, so monitoring can alert on the failure category alone; `mirrored` mode still refuses to run without a remote. Ordinary members cannot read remote addresses, credentials, or another workspace's state.
...
- Filesystem integration tests use local bare remotes to prove coalesced local replication, a remote-behind update, strict first-write publication, rejected strict writes, remote-ahead crash recovery, unborn combinations, restart from a persisted checkpoint, divergence, workspace isolation, and a workspace without a remote reporting replication off rather than a failure. Transport-seam tests prove bounded retry after a timeout and a permanent authentication failure that stops until explicit retry.
