...
- `deploy/tests/run.sh`, wired into `check` as `./gradlew deployScriptTests`, runs the scripts against fake `docker`, `curl`, and `ssh` commands. On Windows the task locates Git for Windows beside `git.exe` and prepends its `usr/bin` to `PATH`, because a non-login Git Bash does not do so itself. It covers the missing-configuration list, digest and revision validation, a live and a stale lock, the registry pull path, the transfer-only tag path, revision mismatch, pull failure, health timeout, an unhealthy container, an unanswering or non-UP entrance, pin recording with an idempotent rerun, settings over standard input, a staged sync that waits for the lock and then swaps both files, a one-run registry login that is never recorded, archive transfer with checksum and staged sync, a remote load failure, and a local image with the wrong revision. The fakes refuse every destructive Compose command.
