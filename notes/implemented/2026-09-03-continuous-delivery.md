...
Registry credentials streamed on standard input log in inside a temporary Docker configuration directory that every Docker and Compose command of that run uses and that is deleted at exit. The directory starts as a copy of the deployment user's `config.json` with the `cli-plugins` and `contexts` directories linked in, so the run keeps the user's context, plugins, proxies, and credential store; the login writes only the copy, and the original is never modified. A run without streamed credentials uses the user's configuration directly.
