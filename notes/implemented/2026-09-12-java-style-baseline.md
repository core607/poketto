...
The suppression list is a debt register. [Executor command ownership](2026-09-16-executor-command-ownership.md) separates CLI operations and transfers from `IsolatedRepositoryExecutor.java`; its remaining lifecycle coordination retains the file exemption. Other entries shrink when their owners are touched. [Shared checks](2026-09-12-shared-checks.md) records the earlier dispatcher split and removal of stale entries, and treats the repeated checks underneath the lengths.
