...
1. Files are the source of truth. Each workspace owns one git repository containing Markdown. Initial selection, superseded by [stock PostgreSQL](2026-09-05-stock-postgresql.md) and [repository-native retrieval](../proposed/2026-09-01-repository-native-retrieval-and-sandboxed-execution.md): PostgreSQL holds only a derived content projection (the search_documents table), rebuildable in full at any time. Each workspace projection records its last processed commit in a checkpoint and catches up by replay after a crash; projection changes and checkpoint advance commit in one database transaction.
