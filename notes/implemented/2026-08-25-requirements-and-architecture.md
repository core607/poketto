...
JDK 26 (fallback: 25 LTS), Spring Boot 4, Spring Modulith (modules: workspace / content / projection / search / web / qa / mcp / auth), Spring AI (MCP server and tool calling), JGit, commonmark-java + Jackson YAML, PostgreSQL 17 + zhparser (requires a custom image, not the stock postgres image), Caffeine, Tailwind on a [proposed Next.js frontend](../proposed/2026-08-30-nextjs-frontend.md) that replaces the original JTE + htmx selection.
