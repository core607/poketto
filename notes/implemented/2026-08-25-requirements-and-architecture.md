...
Files use repository-relative paths without mandatory frontmatter IDs. `get_file` returns authoritative UTF-8 bytes as text, the resolved commit, a service-issued revision, diagnostics, and explicit expected absence. `repo_patch` checks the base commit and each changed path's revision or absence. Images have exact Git or immutable managed versions; uploading neither writes Git nor publishes. Full-read execution sessions retain original Git history; public-only sessions receive current public files without original history or private metadata. Ordinary file edits remain local, while the host-mediated `poketto` CLI authorizes media operations, selected-file saves, atomic moves with reference repair, single-file synchronization and recovery of uncertain writes. Saves preserve unselected local edits and use independent per-file baselines; authoritative reads never read the sandbox copy. The [worker reference](../../executor-service/README.md) defines implemented CLI and lifecycle contracts; the phase-one record retains final deployment acceptance.
