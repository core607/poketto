...
- Used by cloning and self-hosting. The code repository and workspace content repositories are separate: the service git-inits each content repository when its workspace is provisioned; an output-only mirror remote is optional.
...
2. Write model: each workspace repository's local main branch is the truth. Machine entrances (MCP, admin UI) validate frontmatter strictly, serialize writes per repository, and commit on the caller's behalf, recording the caller's identity. The owner may edit and commit in the server worktree as a break-glass path; v1 does not accept an external push into main. The configured remote main is an output mirror and must have no other writer. `poketto.git.acknowledgement=local`, the default, acknowledges the local commit and mirrors it asynchronously; `mirrored` publishes an unpublished candidate under a remote-ref precondition and advances local main only after the remote accepts it. Write results report `committed` and `mirrored` independently so a caller does not retry a durable write blindly; projection lag is reported by the projection boundary rather than changing either observation.
...
Each workspace's Git remote mirrors document text and history but is not an independent backup. Content repositories, image blobs, and non-derived database tables (workspace catalog, keys, audit, budget) each need off-host backup and restore verification; the projection table is not backed up — it rebuilds.
