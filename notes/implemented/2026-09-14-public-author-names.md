...
`SpacePublicationIntegrationIT` verifies article precedence, blank fallback, owner-only updates, independent spaces, the current fallback after a signature change and private-field exclusion over real PostgreSQL; `RepositorySnapshotExportsTests` and `PortableContentPlannerTests` pin the signature in public projections and exports.

Related: [website delivery](2026-09-14-workspace-public-delivery.md) owns public access and [authoring foundations](2026-09-05-repository-authoring-foundations.md) owns source preservation.
