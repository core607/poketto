...
Related: [complete PR review](2026-09-06-complete-pr-review.md) keeps transport completeness for the selected core diff, [repository-aware review](2026-09-09-review-agent-loop.md) the tool loop, and the [original API review decision](2026-09-01-api-pr-review-ci.md) the advisory, non-required status of AI reviews.

`test_session.py`, `test_ci_gate.py` and `test_review.py` under `.github/review` pin scope selection, the CI gate, continuation, rollover and artifact provenance with real Git revisions and scripted providers. Live provider cache behavior requires a paid production call; local evidence does not establish its hit rate.
