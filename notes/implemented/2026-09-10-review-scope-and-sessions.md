...
Successful review artifacts include `session.json`: repository and PR identity, the reviewed base/head, a model/rule/tool contract fingerprint, exact completed conversation messages, and a versioned audit checkpoint. `.github/review/review_session.py` retrieves artifacts only from successful runs of `ai-review.yml` for the current branch, with matching PR/run names. Ordinary PR CI artifacts are not session sources. Dispatch runs must use `main`. Artifacts are decoded as bounded data; no archive paths are extracted or executed. The workflow needs `actions: read`, and review artifacts have 90-day retention. A lookup examines at most 500 successful runs; absent or expired state starts a fresh review. Invalid state or a failed required artifact request fails explicitly.
