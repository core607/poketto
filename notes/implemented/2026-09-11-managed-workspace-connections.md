...
Account-level HTTP operations can connect an existing private GitHub or CNB HTTPS repository as an additional workspace. This implements repository provisioning from [multi-user workspaces](2026-09-11-multiuser-workspaces-and-discovery.md), retaining the [workspace tenant boundary](2026-08-27-workspace-tenancy.md) and [remote repository authority](2026-09-01-remote-repository-authority.md). [Browser workspace routing](2026-09-11-workspace-browser-and-mcp-routing.md) owns its account view and selection. Only accounts whose [site group](2026-09-20-consumer-identity-and-site-policy.md) may create spaces can connect one.
...
Credential rotation shares creation's bounded admission. It validates the replacement outside a database transaction, then rechecks human-owner authorization and compares the expected binding and encrypted credential version before committing. Concurrent rotation or a changed binding rejects the stale result. Fixed error codes cross HTTP boundaries; provider response bodies, tokens, and raw transport exceptions do not. A successful creation removes its extra staged credential copy.
...
- `GET /creation-policy` reports `eligible`, whether the account's group may create spaces, and `available`, whether it is eligible and deployment encryption is configured.
...
Provider-side repository creation with the user's provider token would require broader token authority and recovery of remote resources. Connecting an existing repository keeps that operation with its provider; [GitHub-authorized personal spaces](2026-09-21-github-authorized-personal-spaces.md) add creation through a GitHub App user token as a separate entrance. One database transaction around network validation would hold relational locks while waiting on third parties; a persisted lease releases those locks and prevents a late attempt from completing after another retry takes ownership. URL-only uniqueness would miss repository renames, so immutable provider identity also participates in duplicate detection.

`SpaceCreationHttpIntegrationIT` and the PostgreSQL integration tests pin retry, replay, duplicate rollback, CSRF and foreign-account denial; cipher, provider-parsing and transport tests pin ciphertext binding and unsafe targets. The [provider connection evidence](../../acceptance/evidence/2026-09-15-provider-connection.json) records the Linux probe against a real private repository.
