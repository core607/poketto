...
`DocumentWriteResult` shares the document UUID, the commit that holds the reported state, and the `committed` observation; an operation that finds the repository already in the requested state reports `committed` false and the unchanged commit. `create`, `update`, and `publish` add the resulting path and revision; `delete` reports the removed path and carries no revision, because no blob remains. In the implemented local adapter, the local commit is the acknowledgement point: a crash between commit and response loses only the acknowledgement, and `main` contains either the whole commit or none of it. The proposed [remote repository authority](../proposed/2026-09-01-consumer-multitenancy-stateless-application-and-remote-repository-authority.md) replaces this with one remote ref compare-and-swap acknowledgement model when implemented.
