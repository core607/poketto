...
After the state is healthy, the updater retires the images this installation itself deployed and no longer needs. The state file's `knownImages` records every app/frontend image ID the updater deployed or found running as the previous version; an image that reached the host any other way, including a build an operator loads or pulls by hand and the generations deployed before this record existed, is never touched and stays the operator's to prune. Of the known images, the selected and previous app/frontend images, every image in the rendered configuration and every image a container on the host uses are retained. Removal goes through tags and digests without `--force`; an image Docker declines to delete stays known, and one that is already gone leaves the record. The result reports the count as `retiredImages` and the removed image IDs as `retiredImageIds`; the state file records the same IDs as `retiredImages`, written as each removal is confirmed, so an interrupted retirement still records what it removed. A retirement problem never fails the healthy deployment: it is reported on standard error, as `retirementError` in the result and as `retirementError` in the state file, where it stays until a later retirement completes; the count is then absent while `retiredImageIds` lists the removals that did complete. If the record itself cannot be written, the healthy state already on disk stands and the loss is reported on standard error.
