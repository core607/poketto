...
After the state is healthy, the updater retires images that carry the deployed image's `org.opencontainers.image.source` label and that no pin references. The selected and previous app/frontend images, every image in the rendered configuration and every image a container on the host uses are retained. Removal goes through tags and digests without `--force`; an image Docker declines to delete stays, and a retirement problem is reported on standard error and as `retirementError` without failing the healthy deployment. The result reports the count as `retiredImages` and the removed image IDs as `retiredImageIds`; the state file records the same IDs. Retirement assumes deployments are serialized, which the workflow's concurrency group guarantees; images loaded by a manual delivery that overlaps a running deployment are not protected and must be delivered again.
