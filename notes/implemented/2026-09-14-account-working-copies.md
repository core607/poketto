...
Commands are serialized for a shared copy, including host bridge writes and disposal. Use bounded waiting and distinct capacity/busy reasons. A stopped command releases execution resources while the files remain. Retain per-command sandbox isolation; runtime reuse is a later measurement-based choice.
