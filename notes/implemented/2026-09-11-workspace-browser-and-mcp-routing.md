...
`GET /api/auth/account` identifies the account independently of spaces. `GET /api/auth/workspaces` lists active memberships with deterministic pagination, names, roles and capabilities. `GET /api/auth/workspaces/{workspaceId}/me` rechecks one membership. The workbench uses `workspace` and `tab` query parameters and remounts workspace content when selection changes. [Content navigation](2026-09-14-admin-content-navigation.md) extends the URL with independent folder and document selection. Each component's API client captures its workspace identifier, so an outstanding save cannot switch repositories when a different tab or view selects another space. Unsaved edits require an explicit discard before leaving their view.
...
Invalid or expired invitations receive a locally defined message chosen by the authentication problem code; arbitrary server diagnostics are never displayed as guidance. Successful logout clears workspace and document navigation from the URL, so a subsequent account never inherits the previous account's selected workspace.
...
The browser URL contains workspace identity and navigation only, never repository credentials. Authorization remains necessary after parsing a valid identifier. Public pages select a space by its slug under `/s/{slug}`, never by administration selection; [member permissions](2026-09-12-member-content-permissions.md) govern content and machine access. [Multi-user workspaces and discovery](2026-09-11-multiuser-workspaces-and-discovery.md) own cross-space discovery and public-delivery controls.

## Verification

PostgreSQL and real HTTP integration tests, including a two-space concurrent save into two Git authorities and `McpProtocolIntegrationIT`'s cross-key session rejection, pin membership isolation and credential-derived machine scope; frontend tests cover OAuth space selection and retrying a durable creation request without storing its provider token.
