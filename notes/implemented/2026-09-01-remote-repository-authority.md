...
The adapter does not persist a Git remote in cache configuration and fetches over a direct transport connection that never writes JGit's `FETCH_HEAD`, which would otherwise record the source URI. Binding and configuration string forms are redacted, and transport failures surface sanitized messages without their underlying address or credential. Existing-repository workspace creation and per-workspace binding persistence are specified by [consumer accounts and personal workspaces](../proposed/2026-09-11-multiuser-workspaces-and-discovery.md).
...
- `PublicCacheConcurrencyTests` blocks real-repository fetch and marker-write boundaries to verify that cached public pages and exact images avoid network waits, source protection avoids reverse lock ordering, and observed withdrawal closes publication before marker I/O. The required native Linux replay includes this suite.
