...
The account-level space list supports creation, joining, and switching. Creation asks for a display name, public slug, and an existing GitHub or CNB HTTPS repository with credentials. [Repository initialization on connection](2026-09-16-repository-initialization-on-connection.md) proposes initializing an empty or unguided repository after verification. The creator receives the owner membership. Creation is durable and idempotent, records failure stages, and resumes without duplicate spaces or destructive writes to existing content. One remote repository cannot be bound to two spaces. There is no automatic provider-side repository creation.
