...
SRT 0.0.75 ignores the path-based `allowUnixSockets` setting on Linux; enabling Unix socket creation would require a broader exception. FIFO requests preserve `allowAllUnixSockets: false`. An advisory FIFO writer lock prevents interleaving between cooperative CLI processes; malformed traffic can invalidate only its own lease. Worker lifecycle, signed host polling and protected CLI installation connect `poketto status`, `poketto save`, `poketto recover` and `poketto sync` to the host. Polling does not hold the command operation lock; cancellation preserves the confirmed EXEC result, and command cleanup discards abandoned bridge requests. Media, move and export dispatch remain outstanding.

The [native bridge evidence](../../executor-native/evidence/2026-09-10-public-projection.json) runs the actual CLI through Java, the root worker and SRT. It verifies full/public status, public save denial, selected text commits and explicit deletion against real synthetic Git authority, retained unselected edits, sandbox Git tampering, remote conflicts and recovery from a lost write reply without replaying newer local changes. Single-file synchronization covers conflict markers, deletion intent and unchanged baselines for unselected edits. It also verifies continued Unix socket denial, read-only replies, cross-lease FIFO denial, cancellation, revocation, restart and lease cleanup. Its authentication stub does not replace PostgreSQL or actual HTTP MCP client acceptance.
