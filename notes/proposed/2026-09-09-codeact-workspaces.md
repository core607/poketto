...
The prepared `bridge.LeaseBridge` mailbox uses a separate supervisor-owned Unix socket per lease, a 512-KiB frame bound and four pending connections. It checks the execution UID, assigns request identities and lets the host claim and complete each request once. Closing it interrupts pending reads and replies and joins its listener and handlers. Same-UID lease separation still depends on native sandbox path/socket isolation; mailbox tests do not prove that boundary. Worker lifecycle, signed host polling, CLI installation and service dispatch remain to be integrated.
