...
- `update` takes the document UUID, `expected_revision`, and the full new title, tags, body, and path. It preserves `id`, `created_at`, `published_at`, and visibility. A path change is a move: the UUID is stable, and when the serialized bytes do not change, the revision and `updated_at` do not change either.
- `delete` takes the document UUID and `expected_revision` and removes the document file.
- `publish` takes the document UUID and `expected_revision`, sets visibility to `public`, sets `published_at` on the first publish only, and commits. Publishing an already-public document succeeds without a new commit, so a caller that lost the response retries safely. No operation returns a public document to `private`; reverting is a break-glass repository edit followed by explicit reindexing.
...
Under the lock, the operation reads the current `main` tree, verifies `expected_revision` against the committed blob hash, applies the change, and commits. A mismatch changes nothing and returns a conflict carrying the live revision so the caller re-reads instead of retrying blind. A missing document returns not-found, distinct from conflict: after a lost `delete` acknowledgement, a retry reads as already applied. `create` verifies path availability after Unicode normalization and case folding, and document-UUID uniqueness, under the same lock. The first successful write on an unborn `main` creates the root commit.

The write result carries the document UUID, path, revision, and commit SHA together with the `committed` observation. The acknowledgement policy and the `mirrored` and `indexed` observations belong to [Git replication and write acknowledgement modes](2026-08-27-git-durability-modes.md); these operations are the machine write its policies wrap. A crash between commit and response loses only the acknowledgement — `main` either contains the whole commit or none of it.
...
- `publish` sets `published_at` exactly once; republishing is an acknowledged no-op; no operation makes a public document private.
- A move without content change preserves revision and `updated_at`; a content change advances both.
