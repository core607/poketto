...
- PostgreSQL stores no document body, document metadata, search vector, content-search index, projection checkpoint, or index-lag state. It remains the store for relational application data such as workspaces, accounts, memberships, invitations, API-key metadata, asset catalog records, synchronization operations, audits, and budgets.
- All repository reads resolve a workspace and a committed remote `main` revision through [remote repository authority](2026-09-01-remote-repository-authority.md) before reading. A request observes that commit even if a later write advances `main` while the request is running. Managed image reads independently pin the opaque asset revision returned by the asset catalog.
...
The trusted-agent MCP read surface uses `repo_exec` for composable exploration plus bounded structured reads for exact files and images. `repo_exec` replaces the fixed `search`, `list_tags`, `list_recent`, and `history` tools. [Repository-native publishing and assets](2026-09-01-repository-native-publishing-and-assets.md) owns `get_file`, `get_asset`, and the separate `repo_patch` write bridge; command execution never writes back to authority.
...
An operating-system service manager runs a local executor service under a dedicated account distinct from the Spring application account. Spring submits bounded jobs over a permissioned local socket. Application repository caches remain owned by the application account and grant the executor account read and traverse access only. The executor creates the execution workspace before launching the sandbox, using `git clone --no-local` or an equivalently verified full object copy. It rejects a clone with an alternates file, symlinked object storage, or any object file sharing an inode with the source cache. Agent commands never run as the application account, and no authority credential, cache object database, or asset BlobStore is mounted into their sandbox.
...
- Filesystem reads are denied except for the session workspace, the declared toolchain and runtime libraries, and per-invocation temporary storage. The application repository cache, other workspaces, asset BlobStore, application configuration, credentials, user profiles, database files and sockets, container runtime sockets, and service control interfaces are inaccessible.
...
The first implementation depends on the implemented [workspace boundary](../implemented/2026-08-27-workspace-tenancy.md), [remote repository authority](2026-09-01-remote-repository-authority.md), and [repository-native publishing and assets](2026-09-01-repository-native-publishing-and-assets.md). It includes committed-tree reads that tolerate per-file validation failures, bounded in-memory search, `get_file`, `get_asset`, the execution capability and MCP contract, session-scoped commit-pinned workspaces, the separate-account sandbox adapter, resource enforcement, cleanup recovery, audits, and focused security and isolation tests.
...
- PostgreSQL contains no content-search, document, projection, checkpoint, or index-lag state. Restoring the authoritative relational rows, remote workspace repositories, and asset BlobStore restores repository and managed-image reads without a projection or indexing job.
...
- `get_file` returns exact UTF-8 content and its opaque blob revision from one committed repository snapshot. `get_asset` returns one confirmed managed asset revision plus optional Git binding and synchronization state. A `repo_exec` result can supply the resolved commit for path-based lookup, and no agent-facing contract requires or permits callers to derive a repository or asset revision from file bytes.
