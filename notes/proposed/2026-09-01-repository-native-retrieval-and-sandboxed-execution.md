...
- All content reads resolve a workspace and a committed remote `main` revision through [remote repository authority](2026-09-01-remote-repository-authority.md) before reading. A request observes that commit even if a later write advances `main` while the request is running.
...
- PostgreSQL contains no content-search, document, projection, checkpoint, or index-lag state. Restoring the authoritative relational rows and remote workspace repositories restores all content reads without a projection or indexing job.
...
- `get_file` returns exact UTF-8 content and its opaque blob revision from the same committed snapshot; `get_asset` returns bounded image content from that snapshot. A `repo_exec` result can supply the resolved commit, and no agent-facing contract requires or permits callers to derive a revision from file bytes.
