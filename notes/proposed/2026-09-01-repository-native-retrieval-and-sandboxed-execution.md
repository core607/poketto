...
An operating-system service manager runs a local executor service under a dedicated account distinct from the Spring application account. Spring submits bounded jobs over a permissioned local socket. Workspace repositories remain owned by the application account and grant the executor account read and traverse access only. The executor creates the execution workspace before launching the sandbox, using `git clone --no-local` or an equivalently verified full object copy. It rejects a clone with an alternates file, symlinked object storage, or any object file sharing an inode with the source repository. Agent commands never run as the application account, and the authoritative repository path is not mounted into their sandbox.
...
- The executor runs as an account distinct from the application repository owner, has read-only source access, and creates a full-copy clone with no alternates or shared object inodes. A security test modifies permissions and bytes under the clone's object database; the source repository remains byte-identical and `git fsck` succeeds.
