...
- An `account` is a human identity. A `workspace` remains the tenant, remote repository, derived asset namespace, quota, audit, backup, and data-destruction boundary.
...
Provisioning is a durable state machine because an account row, workspace catalog row, membership, and content repository cannot be created in one storage transaction. Every step is idempotent and keyed by a recorded provisioning identifier and `WorkspaceId`. Duplicate delivery resumes the same operation; reuse of an idempotency key for different registration input fails.

A workspace is not routable until its relational state and remote repository authority report ready. Failed provisioning remains visible only to the affected account and instance operator. Cleanup removes only resources created for that `WorkspaceId`; it never guesses ownership from provider names or paths. The derived repository-asset BlobStore may initialize lazily because its objects can be rebuilt from Git and do not decide workspace readiness.
...
The first implementation depends on the account, session, and membership foundation from [invitation-only membership](2026-08-27-invitation-only-membership.md) and the binding contract from [remote repository authority](2026-09-01-remote-repository-authority.md). It adds the provider adapter that creates an isolated private repository, durable provisioning state, personal-workspace creation entrance, owner membership, retry and cleanup behavior, authorization, audit events, and focused failure-injection and isolation tests.
...
- Duplicate and concurrent delivery creates exactly one account, personal workspace, `OWNER` membership, and private remote repository for one provisioning operation.
...
- The implementation exercises an isolated provider repository fixture with PostgreSQL under duplicate delivery, injected failure, ambiguous provider response, restart, and two-account isolation tests.
