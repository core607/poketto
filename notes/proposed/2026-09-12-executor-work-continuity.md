...
The worker's [checkpoint protocol](../../executor-service/README.md#worker-checkpoints) supplies immutable host files and explicit source-lease containment. It retains the trusted original export separately from mutable Git state, verifies complete bytes before extraction and requires a fresh restore lease. It is not yet called by the application. Application command-boundary persistence, generation ownership, required later baseline objects and expiry collection remain integration work; this primitive does not change the MCP acknowledgement contract by itself.
...
Host records now carry the latest worker boot and lease; changing either requires a new generation. The metadata store supplies bounded per-copy writer locks independently of its short metadata lock. Every lock acquisition first excludes same-JVM contenders before opening a channel: on Linux, closing a competing channel can release the original process lock, as described by the [Java file-lock contract](https://docs.oracle.com/en/java/javase/26/docs/api/java.base/java/nio/channels/FileLock.html). Writer files are never removed while held. Unowned files without a record are reclaimed only under the metadata lock that also guards writer acquisition, so a new holder cannot open a replacement inode while a contender still targets the old one. These mechanisms remain unwired to application commands.

An unconfirmed command retains its own paired worker checkpoint and host state, separately from the last acknowledged command. `CHECKPOINT_ACTIVE` captures a running command with its cgroup frozen and returns RUNNING with the matching execution ID. The application must use these intermediate points at host-effect boundaries: preserve pending write payload and the exact candidate before pushing, then pair completed save, sync, move or import state with matching worker bytes before returning its bridge reply. A successful save inside an interrupted command must not restore an older file under the saved file's newer baseline. The Java request and response records cover these points and require the checkpoint capability for retained execution; the command coordinator still needs to invoke them.
