...
Real repository tests must cover README-only folders, index precedence, excluded/private indexes, relative folder links and untouched raw source. The running frontend must demonstrate a named folder gallery, previous/next by mouse and keyboard, focus containment, Escape return, image failure and mobile layout. Existing public withdrawal and private-image authorization remain required checks.
