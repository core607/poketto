...
Under [remote repository authority](2026-09-01-remote-repository-authority.md), Markdown and optional Git asset sources already live off the application host. Local and OSS [BlobStores](2026-09-01-repository-asset-blob-store.md) are authoritative for managed image bytes, including images with no Git source. Backup must protect the remote repository, BlobStore, and non-derived PostgreSQL state in independent recovery boundaries.

## Proposal

### Common rules

- Production configuration names recovery targets independent from the authoritative Git provider, asset BlobStore, and PostgreSQL service. A target may be a self-hosted service or operator-selected third-party storage. Normal application operation does not require continuous backup-target availability.
...
- Optional Git asset sources are covered with the commits that refer to them. They aid asset recovery but do not replace the BlobStore recovery point.

### Asset BlobStores

- Backup copies every live immutable asset object plus a verifiable manifest containing workspace, managed asset identity, content hash, byte size, media type, and catalog checkpoint. It retains versions long enough to cover delayed deletion and synchronization recovery.
- Restore verifies every object hash before making it available. A Git-backed asset may be reconstructed from its recorded source revision when that source is still reachable, but managed-only assets require the BlobStore recovery point. Missing or corrupt bytes are reported rather than replaced with a placeholder or re-encoded image.
...
- Accounts, memberships, invitations, API-key metadata, audits, budgets, workspace catalog, asset catalog, Git bindings, and pending synchronization operations are non-derived data that must be recoverable. Secrets retain their hashed or encrypted storage contract.
...
Content backup depends on implemented remote repository authority, asset backup depends on implemented BlobStore and catalog contracts, and database backup depends on the first non-derived tables and schema-management mechanism. These media have no common implementation prerequisite and therefore become separate tasks behind their own dependencies. Production automatic deployment may check a recent verified backup only after all three exist and pass one combined restore drill. Derived repository and execution caches add no backup task.
...
**Treat the authoritative Git provider as the backup.** Git contains repository text, optional asset sources, and history, but provider deletion, credential compromise, or a mistaken ref update can damage the same authority. It contains neither managed-only images nor authoritative PostgreSQL rows.
...
**Encrypt backups before they leave the host.** Client-side encryption would protect content from the storage provider, but key custody becomes an independent recovery prerequisite — a lost key silently turns every recovery point unreadable — and the authoritative content Git repository would still carry plaintext, leaving mixed guarantees. v1 keeps backups readable with the operator's target credentials alone; a later proposal may add encryption together with a key-custody and restore-drill story.

## Acceptance

- Remote repository authority, managed BlobStore assets, and non-derived PostgreSQL data can retain versioned, verified recovery points in independent failure domains. A missing target fails explicitly and cannot report success.
...
- A restore into an empty application data directory produces a replacement remote repository whose commits match the repository manifest, BlobStore objects whose hashes match the asset manifest, and non-derived database constraints matching their manifest.
- Restore excludes disposable repository, read, and execution caches. The first repository-backed read resolves restored remote `main`; managed-only and Git-backed images resolve their confirmed catalog versions without a database indexing prerequisite.
