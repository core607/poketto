...
- Own the content worktree at `<data-dir>/content`. Other durable data may gain sibling directories later; it does not belong inside the content repository unless the requirements explicitly say so.
...
Tests provide their own temporary absolute data directories. Implementing this proposal also updates the local run documentation so `bootRun` supplies or explains the required setting.
...
- `id` is a canonical lowercase UUID and is immutable after creation. The content layer enforces repository-wide uniqueness.
...
- Expose immutable content-module value types for document ID, revision, visibility, metadata, and document content. Keep JGit and YAML implementation classes below `content.internal`.
...
The first implementation adds configuration binding, repository bootstrap and validation, document parsing and canonical serialization, value types, tree scanning, and focused tests. It does not add create, update, delete, publish, projection, HTTP, or MCP entry points. Those operations will build on this boundary in later short-lived changes.
...
- Repository tests cover absent, empty, valid existing, non-empty non-repository, bare, unreadable, wrong-branch, and unborn-`main` cases without using the developer's real data directory.
...
- Spring Modulith verification continues to pass and JGit or YAML implementation types are not exposed as content-module contracts.
