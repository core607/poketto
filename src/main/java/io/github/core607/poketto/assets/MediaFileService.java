...
public final class MediaFileService {
    ...
    private Download memberPublicDownload(
        ...
        try (var admission = admit(workspace, false)) {
            ...
            if (requested.isPresent() && !requested.get().equals(commit)) throw missing();
            var catalog = repository.media(workspace, commit);
            if (!catalog.publicPaths().contains(path)) throw missing();
            ManagedAsset asset = resolve(workspace, catalog.index().files().get(path));
            Runnable check = () -> {
                identity.run();
                snapshots.withCurrent(workspace, current -> {
                    if (!current.commit().equals(Optional.of(commit))) throw missing();
                    return null;
                });
            };
            check.run();
