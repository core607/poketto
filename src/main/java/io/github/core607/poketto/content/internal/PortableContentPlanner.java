...
final class PortableContentPlanner {
    ...
    Plan prepare(AuthPrincipal actor, WorkspaceId workspace, List<String> selections, boolean publicOnly) {
        ...
        for (String path : selections)
            if (!path.isEmpty()) {
                ...
                if (internal(path)) throw unavailable();
            }
        ...
        if (!tree.workspaceId().equals(workspace)) throw unavailable();
        String commit = tree.commit().orElseThrow(PortableContentPlanner::unavailable);
        var media = blobs.media(workspace, commit);
        if (!media.workspaceId().equals(workspace) || !media.commit().equals(commit)) throw unavailable();
        ...
        if (publicOnly
                && (!published.workspaceId().equals(workspace)
                        || !published.commit().equals(Optional.of(commit)))) throw unavailable();
        Map<String, PublicArticle> publicArticles = new HashMap<>();
        if (publicOnly) published.articles().forEach(article -> publicArticles.put(article.repositoryPath(), article));
        ...
        Runnable check = () -> {
            ...
            if (publicOnly)
                snapshots.withCurrent(workspace, current -> {
                    if (!admitted.equals(fingerprint(workspace, current))) throw unavailable();
                    return null;
                });
        };
        ...
        for (String selection : selections) {
            ...
            for (var diagnostic : tree.diagnostics())
                if (inside(diagnostic.path(), selection)
                        ...
                        && !diagnostic.code().equals("INFERRED_METADATA")) throw unavailable();
            for (var document : tree.documents()) {
                String path = document.file().path();
                if (!inside(path, selection) || internal(path)) continue;
                if (!document.file().workspaceId().equals(workspace)
                        || !document.file().commit().equals(Optional.of(commit))) throw unavailable();
                if (publicOnly && !publicArticles.containsKey(path)) throw unavailable();
            ...
            if (matches == 0) throw unavailable();
        }
        ...
        for (String path : selectedMedia) builder.indexed(path);
        for (var entry : selected.entrySet()) {
            ...
            if (publicOnly) {
                ...
                if (!source.endsWith(body)) throw unavailable();
            ...
            if (bytes.length > ContentLimits.MAX_DOCUMENT_BYTES
                    || bytes.length > ContentLimits.MAX_WORKSPACE_BYTES - builder.textBytes) throw unavailable();
    ...
    private final class Builder {
        ...
        Builder(
            ...
            for (var entry : selected.entrySet()) {
                ...
                if (routes.containsKey(route)) routes.put(route, "");
                else routes.put(route, target);
            }
        }
        ...
        String destination(String source, String archive, String authored) {
            ...
            if (target == null && raw.startsWith("/")) target = routes.get(raw);
            if (target != null && !target.isEmpty()) return relative(archive, target) + fragment;
            if (raw.startsWith("/") && articleRoutes.contains(raw)) return authored;
            if (media.index().files().containsKey(path)) return relative(archive, indexed(path)) + fragment;
            ...
            if (RepositoryPathRules.markdown(path)) {
                if (publicOnly && !published.containsKey(path)) throw unavailable();
                return authored;
            }
            String key = "git:" + path;
            if (!originalPaths.containsKey(key)) {
                ...
                if (!blob.workspaceId().equals(workspace)
                        ...
                        || (publicOnly && !blob.publicPath())) throw unavailable();
        ...
        String nextMedia(long size, String extension) {
            if (size > MAX_MEDIA_BYTES - mediaBytes) throw unavailable();
            mediaBytes += size;
            return "media/original-" + (originalPaths.size() + 1) + extension;
        }

        void bound() {
            if (entries.size() > 10_000) throw unavailable();
        }
    }

    private String fingerprint(WorkspaceId workspace, PublicContentSnapshot snapshot) {
        if (!snapshot.workspaceId().equals(workspace)) throw unavailable();
    ...
    private static Set<String> referencedPaths(Collection<PublicArticle> articles) {
        ...
        for (var article : articles) {
            ...
            java.util.stream.Stream.concat(refs.links().stream(), refs.images().stream())
    ...
    private static String publicBody(String body, java.util.function.UnaryOperator<String> destination) {
        ...
        while (!pending.isEmpty()) {
            ...
            if (nodes.size() >= 20_000 || visit.depth() > 128) throw unavailable();
        ...
        for (Node node : nodes) {
            if (node instanceof HtmlBlock || node instanceof HtmlInline || node instanceof LinkReferenceDefinition)
                ...
                if (link.getDestination().startsWith("managed:")) throw unavailable();
                String target = destination.apply(link.getDestination());
                if (safeDestination(target)) link.setDestination(target);
                else flatten(node);
            } else if (node instanceof Image image) {
                String target = destination.apply(image.getDestination());
                if (safeDestination(target)) image.setDestination(target);
                else flatten(node);
            }
        }
