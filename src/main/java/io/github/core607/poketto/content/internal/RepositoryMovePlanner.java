...
final class RepositoryMovePlanner {
    static RepositoryCandidateChanges prepare(
        ...
        for (var file : files.entrySet()) {
            ...
            if (routes.putIfAbsent(metadata.route(), path) != null) {
                ambiguousRoutes.add(metadata.route());
            }
        }
    ...
    static Map<String, String> relocate(Set<String> namespace, RepositoryMoveRequest request) {
        ...
        if (moved.isEmpty()) {
            throw new IllegalArgumentException("move source does not exist");
        }
        ...
        for (String path : namespace) {
            ...
            if (!finalPaths.add(newKey)) {
                throw new IllegalArgumentException("move creates a path collision");
            }
        }
