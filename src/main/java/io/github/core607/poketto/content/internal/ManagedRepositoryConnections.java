...
final class ManagedRepositoryConnections implements RepositoryConnections, AutoCloseable {
    ...
    private void rejectDefaultDuplicate(RepositoryProviderClient.Metadata candidate) {
        if (configured.remoteUri() == null) return;
        ...
        RepositoryCoordinates operator;
        try {
            operator = RepositoryCoordinates.parse(configured.remoteUri());
    ...
    public void rotate(WorkspaceId workspace, String username, String token) {
        requireTransaction();
        var rows = jdbc.query(
                "select canonical_uri,provider_identity from content_repository_bindings where workspace_id=? for update",
                (row, number) -> List.of(row.getString(1), row.getString(2)),
                workspace.value());
        if (rows.isEmpty()) throw new RepositoryConnectionException(UNAVAILABLE);
        var coordinates = RepositoryCoordinates.parse(rows.getFirst().getFirst());
        byte[] sealed = seal(workspace, coordinates, username, token);
        var verified = verify(workspace, coordinates, sealed);
        if (!verified.providerIdentity().equals(rows.getFirst().get(1)))
            throw new RepositoryConnectionException(REPOSITORY_CHANGED);
        jdbc.update(
                "update content_repository_bindings set sealed_credentials=?,updated_at=current_timestamp where workspace_id=?",
                sealed,
                workspace.value());
    }
