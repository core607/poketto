...
- 公开与私有分离：目录表达发布状态，发布受权限控制；公开范围的 Agent 使用不包含私密文件及原始 Git 历史的独立投影。空间可以选择向读者公开文章此前的公开版本。
...
- 浏览器创作：本机草稿恢复、已保存版本对比与恢复、图片粘贴、预览与明确的发布和撤回操作；发现结合作者精选、时间、标签和有界随机浏览，关注动态独立归属个人账号。
- GitHub 个人空间：具备资格的用户授权站点配置的 GitHub App，在自己的账号下创建私有仓库；日常访问限定到该仓库。[接入与恢复](docs/usage.zh.md#内容与图片)。
- 媒体播放：支持的索引原件提供原生音视频控件，同时保留下载入口。
...
[参与开发](AGENTS.md) · [客户端验收](acceptance/clients/README.md) · [沙箱验证](executor-native/README.md) · [交付范围](notes/implemented/2026-09-05-phase-one-daily-use.md)
