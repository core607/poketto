...
使用 Java 26 和仓库内的 Gradle Wrapper。数据库集成测试与完整校验需要 Docker；较快的单元测试和仓库校验不需要。`check` 还使用[锁定的前端运行时](frontend/README.md)。Linux 执行服务测试要求 Python 3.10+ 及 venv、pip 支持，将锁定依赖安装到构建私有环境；Windows 在锁定的 Linux 容器中运行这些测试。完整校验除了部署脚本样例，也会在锁定容器中验证真实 Caddy 配置。
