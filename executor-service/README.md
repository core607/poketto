...
Setting `poketto.executor.retention.enabled=true` requires absolute `poketto.executor.retention.root` and `poketto.executor.retention.baseline-root` directories for metadata and original archives. The roots must use persistent host storage, have safe existing parents, remain private to the application account and neither contain the other. This mode requires worker checkpoint protocol 1; an unavailable capability never falls back to ephemeral execution. Retention remains disabled by default pending complete failure, storage and client acceptance. Private-copy admission captures its original archive and binds it into metadata; using that archive for subsequent baseline lookup remains integration work.
...
Explicit recovery transfers the expected generation after confirming the prior writer's containment and restores the selected checkpoint before executing the same request. Superseded points are removed only after publication; cleanup failures preserve the new point. Worker startup still cleans disposable mounts. Original-baseline lookup, later non-text baselines, further crash-window evidence and actual-client acceptance remain incomplete. [Work continuity](../notes/proposed/2026-09-12-executor-work-continuity.md) remains proposed until that remaining work is verified.
...
In retained mode, acknowledged saves and synchronizations persist each selected file's exact baseline text or absence with its commit. Sync retains the remote input, independently of its merged local output. Moves retain affected remote text with their confirmed plan and adopt it after local installation; unselected draft media mappings never become the index baseline. Later operations can use these versions after historical cache reads become unavailable, while current authorization and remote conflict checks still apply. Original and non-text baseline lookup remains incomplete; retention stays disabled by default under the [continuity proposal](../notes/proposed/2026-09-12-executor-work-continuity.md).
