...
The application retries only an explicit `CHECKPOINT_UNAVAILABLE / BUSY` capture refusal, which occurs before checkpoint creation. Attempts keep the same checkpoint ID, recheck authorization and share the original 30-second capture budget. A lost response, capacity error or uncertain publication is never automatically retried. If capture still fails, command containment and recovery of the last acknowledged point remain required.
