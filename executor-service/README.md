...
`CHECKPOINT_ACTIVE` adds the matching `executionId` and requires RUNNING. It serializes with materialization and capture through the files lock, freezes the entire command cgroup while retaining bytes, then thaws. Its response remains RUNNING and echoes the execution ID; it is an intermediate recovery point, not a completed command acknowledgement. The application must pair this point with its host save state before acknowledging a bridge operation or allowing a remote write. Freeze, persistence, cancellation or revocation failure prevents a checkpoint acknowledgement.
...
The application retries only an explicit `CHECKPOINT_UNAVAILABLE / BUSY` refusal before capture, restore or checkpoint removal starts. Attempts retain the same checkpoint and lease references, use a fresh wire request ID and recheck authorization. Capture shares its original 30-second budget; restore shares its initialization budget; each checkpoint removal has a five-second cleanup budget. A lost response, capacity error or uncertain publication is never automatically retried. If capture still fails, command containment and recovery of the last acknowledged point remain required. Cleanup that cannot finish within its budget leaves the unreferenced checkpoint for expiry collection; it cannot revoke an acknowledged replacement.

Checkpoint file slots and retained-copy slots are different limits. The example provides 128 checkpoint slots for the application's default 32 retained copies, allowing acknowledged and intermediate points, their replacements and cleanup headroom. Byte quotas and disk reserves still apply independently; this does not promise that every copy can reach its individual byte limit.

A signed `RESTORE` request uses a fresh lease ID and carries `checkpointId`, `sha256`, `bytes`, `commit`, `scope` and `previousLeaseId`. The application must retain and supply the latest writer lease, which can differ from the checkpoint's source after an interrupted restored command. The worker verifies owner, account, workspace, scope, original commit, expiry and the complete checksum before extracting into a new private staging directory. It stops both source and supplied writer leases across application boots and refuses admission until their process groups are empty and their working mounts have closed. A checkpoint cannot have two active restored leases in one worker. Old lease execution is refused. `CHECKPOINT_REMOVE` uses the same three-field checkpoint reference and original identity; explicit removal remains possible after expiry. Expired checkpoints cannot restore; periodic expiry collection is described below.
...
Original archives use `max-baseline-bytes` (268435456), `max-baseline-expanded-bytes` (1073741824), `max-baseline-entries` (100000) and `max-baseline-total-bytes` (2147483648), under the same prefix. They share `max-copies`, `disk-reserve-bytes` and the copy's fixed expiry. Their aggregate quota is separate from metadata and worker checkpoints; provision all three stores together. Archive creation has its own nonblocking lock, so compression does not hold the metadata lock or prevent existing original reads. A successful archive publication follows file fsync, atomic rename and directory fsync; a failure after rename is uncertain and must not be acknowledged as a completed publication.

An independent original-archive collector runs on startup and once a minute. It removes abandoned staging files only under the archive lock. Published archives require a checked header and matching owner/copy address; deletion also holds the copy's metadata writer lock and rechecks that metadata is absent or expired. Active writers and live copies prevent removal. Maintenance can acquire an orphan's writer even when ordinary writer slots are full, so capacity exhaustion does not prevent reclamation. Corrupt or aliased entries are preserved for diagnosis. Store readers reject expired authority and released writers; current user authorization remains the caller's responsibility.

An application collector scans retained metadata on startup and once a minute on a thread separate from lease renewal. It verifies each bounded frame and its identity, then removes expired records only after acquiring their writer lock. Busy commands and recovery defer deletion; corrupt records remain for diagnosis. Active retained leases close at their fixed expiry without waiting for MCP idle expiry. Failed initial storage admission releases its unacknowledged transport slot; rejecting a stale generation on an existing copy preserves that copy.

The worker independently checks checkpoint storage once a minute under the checkpoint lock. It validates private file ownership, the bounded header and its owner/ID filename before removing expired checkpoints. Interrupted `.pending-` files can be removed only while that lock is free. Busy capture or restore defers collection; invalid headers or aliased files are not deleted. Neither collector extends the recovery window, and cleanup does not restore expired work from current Git history. After collection an expired ID may report `MISSING_COPY` instead of `EXPIRED`.

Retained commands hold a per-copy writer lock, persist intent before EXEC, capture paired worker/host state at save, sync, move and import boundaries, and publish a READY checkpoint before returning a normal result, including nonzero exits. Private admission first captures the authorized original baseline at the worker's pinned commit, then captures its initial worker checkpoint and publishes the metadata before EXEC. The archive reference is stored once, binds the copy owner, commit and fixed expiry, and cannot change during generation transfer. Failed capture or metadata publication prevents acknowledgement; an unacknowledged archive is collected after its writer releases. Public copies retain their host-owned projection proof and cannot carry a private archive reference.

Explicit recovery transfers the expected generation after confirming the prior writer's containment and restores the selected checkpoint before executing the same request. Superseded points are removed only after publication; cleanup failures preserve the new point. Worker startup still cleans disposable mounts. [Work continuity](../notes/proposed/2026-09-12-executor-work-continuity.md) records native crash-window and authenticated model-client evidence. It remains proposed pending final HTTPS acceptance and the resource/admission work.
...
`SESSION_REPLACED` means this command did not execute. `MISSING_COPY` permits intentional new admission in the current transport (`newCopyAllowed: true`); `DIFFERENT_COPY` supplies the current authorized ID; `CLOSED_COPY` permits explicit `new` in the same transport only when `newCopyAllowed` is true: the worker has confirmed lease release and the previous command has exited. Explicit `new` never resets a live copy or bypasses unconfirmed cleanup. A verified HELLO from a different worker boot establishes exclusive startup cleanup even when the pool has spare capacity. The adapter checks stopped copies before refusing their old IDs; missing or invalid HELLO and an old command that has not exited keep new admission closed. These ephemeral-copy refusals report `recoveryAvailable: false`. Do not replay an uncertain write or automatically replace the expected ID with `new`.

With retention enabled, successful results also contain `retention`: `generation`, fixed `expiresAt` in epoch milliseconds, `resumed`, and nullable `lastInterruptedCommand`. Pass the observed generation as `expectedGeneration` on every continuing call. After reconnecting, use the same ID and generation with `resume: true` and a read-only command such as `poketto status`. Recovery and that command share one request; no subsequent transport reuse is required. Recovery restores the paired work tree and host state at their original pinned commit, fences the previous lease using its retained worker and application boot identities, and advances the generation before starting the restored lease. It never claims a copy merely because the principal and workspace match. A different live copy already bound to the receiving transport is not replaced.

`EXECUTION_REFUSED` reports `executed: false`, a bounded `reason`, nullable `currentGeneration` and `recoveryAvailable`. Reasons distinguish `RECOVERY_REQUIRED`, `GENERATION_MISMATCH`, `BUSY`, `CAPACITY`, `MISSING_COPY`, `EXPIRED`, and `UNAVAILABLE`. The refusal describes only this request; it never proves an earlier write failed. Re-read the generation after a refused or interrupted transfer, inspect the intended recovered copy, and reconcile uncertain saves before replaying writes. `lastInterruptedCommand` remains visible in later results; it identifies possible partial work, not an instruction to repeat the command. Missing or expired records do not authorize reconstruction against current main. Recovery does not restore session artifacts or `/tmp`.
...
Retained file versions explicitly distinguish text, non-text presence and absence. Private retained checkpoints must cover every advanced path and every path in an acknowledged pending move. Non-text versions preserve their available exact-byte revision and diagnostics, so restored media checks can distinguish Git collisions from indexed media. Diagnostics are bound to their file path and remain inside the metadata storage quota. Text saves return `NO_WRITABLE_BASELINE` for existing non-text targets before creating a pending write intent.
