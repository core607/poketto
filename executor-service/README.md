...
Setting `poketto.executor.retention.enabled=true` and an absolute `poketto.executor.retention.root` enables application command checkpoints and explicit recovery. The root must use persistent host storage, have a safe existing parent and remain private to the application account. This mode requires worker checkpoint protocol 1; an unavailable capability never falls back to ephemeral execution. Retention remains disabled by default pending complete failure, storage and client acceptance.

Application retention bounds are `max-copies` (32), `max-record-bytes` (100663296), `max-total-bytes` (536870912), `disk-reserve-bytes` (1073741824) and `seconds` (86400), all under `poketto.executor.retention`. These bound host metadata separately from worker checkpoint storage. A copy's expiry is fixed at admission. Deployment must size both stores; these defaults are not a promise that all copies can reach their individual byte limits simultaneously.
