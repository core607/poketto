...
Setting `poketto.executor.retention.enabled=true` and an absolute `poketto.executor.retention.root` enables application command checkpoints. The root must use persistent host storage, have a safe existing parent and remain private to the application account. This mode requires worker checkpoint protocol 1; an unavailable capability never falls back to ephemeral execution. Retention is disabled by default while the MCP recovery entrance remains incomplete.
