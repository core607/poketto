...
Setting `poketto.executor.retention.enabled=true` requires absolute `poketto.executor.retention.root` and `poketto.executor.retention.baseline-root` directories for metadata and original archives. The roots must use persistent host storage, have safe existing parents, remain private to the application account and neither contain the other. This mode requires worker checkpoint protocol 1; an unavailable capability never falls back to ephemeral execution. Retention remains disabled by default pending complete failure, storage and client acceptance. The archive store is provisioned, but its capture and lookup are not yet bound into executor recovery.
...
Retained commands hold a per-copy writer lock, persist intent before EXEC, capture paired worker/host state at save, sync, move and import boundaries, and publish a READY checkpoint before returning a normal result, including nonzero exits. Failed capture or metadata publication prevents acknowledgement. Superseded points are removed only after publication; cleanup failures preserve the new point. Public records also retain their host-owned projection proof. Worker startup still cleans disposable mounts, and MCP reconnects cannot yet restore work: expected-generation ownership transfer, atomic resume-and-execute, later baseline objects and expiry/orphan collection remain incomplete. [Work continuity](../notes/proposed/2026-09-12-executor-work-continuity.md) remains proposed pending those integrations and actual-client acceptance.
