...
Use Java 26 and the checked-in Gradle Wrapper. Docker is required for database integration tests and the complete check; the faster unit and repository checks do not require it. Linux executor tests require Python 3.10+ with venv and pip support; Windows runs the required protocol/state suite in a pinned Linux container.
