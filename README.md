...
The [content plan](notes/proposed/2026-09-09-codeact-content-and-media.md) still includes default-private `public/` and `private/` roots, CLI ZIP exports, content conversion and removal of redundant MCP file tools. The current publication policy and tool interfaces are documented in the [usage reference](docs/usage.md). Those planned changes are not available yet. The browser can export selected saved documents and media as private or public ZIP packages with actual originals and relative links.
