...
`poketto media import` stores a workspace-owned immutable original and updates its local logical index; save that index with referring text to persist the references. `poketto media fetch` uses the local index or an explicitly selected historical commit in full-read sessions, and the host-owned approved mapping in public sessions. CLI paths are repository-relative; use `poketto --help` for commands and file lifetime. The [worker reference](executor-service/README.md) owns limits, permissions, conflict behavior and coordinated worker installation. Artifact returns, dedicated CLI moves, portable exports and removal of redundant MCP tools remain in the [content plan](notes/proposed/2026-09-09-codeact-content-and-media.md).
