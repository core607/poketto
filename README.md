...
Use Java 26 and the checked-in Gradle Wrapper. The frontend and complete check also require Node.js 24.19.0 and npm 12.0.2. Docker is required for database integration tests and the complete check; the faster unit and repository checks do not require it. `./gradlew frontendCheck` runs frontend formatting, types, tests and the production build. Use the [isolated browser entrance](acceptance/README.md) to exercise the real application with synthetic data; frontend runtime settings are documented in [frontend/README.md](frontend/README.md). Linux executor tests require Python 3.10+ with venv and pip support; Windows runs the required protocol/state suite in a pinned Linux container.
